...
def generate_synthetic_demo_artifacts(
    ...
    *,
    ...
    LiDAR maps, compressed splatted maps, enriched baseline model inputs, and a
    JSON manifest.
    """
    ...
    model_input_dir = output_root / "model_inputs"
    for directory in (overlay_dir, sparse_dir, splat_dir, model_input_dir):
    ...
    model_batch = dataset_items_to_model_batch(
        [
            {
                "image": np.moveaxis(image.astype(np.float32) / 255.0, -1, 0),
                "lidar_maps": lidar_maps,
            }
        ],
        splatting_config=splat_config or SplattingConfig(radius_px=2, sigma_px=1.0),
    )
    model_inputs = model_batch_to_baseline_inputs(model_batch, lidar_mode="enriched")
    ...
    model_inputs_path = model_input_dir / "synthetic_baseline_inputs.npz"
    ...
    np.savez_compressed(
        model_inputs_path,
        rgb=model_inputs["rgb"],
        lidar_maps=model_inputs["lidar_maps"],
    )
    ...
    manifest: dict[str, Any] = {
        ...
        "model_input_shapes": {
            "rgb": list(model_inputs["rgb"].shape),
            "lidar_maps": list(model_inputs["lidar_maps"].shape),
        },
        "outputs": {
            ...
            "model_inputs": model_inputs_path.relative_to(output_root).as_posix(),
        },
        "artifact_integrity": {
            ...
            "model_inputs": _artifact_integrity(model_inputs_path),
        },
    }
